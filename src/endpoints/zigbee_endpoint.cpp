...
// Initialize endpoint id counter
uint8_t ZigbeeEndpoint::m_ep_id_counter = 0;

ZigbeeEndpoint::ZigbeeEndpoint(zb_char_t model_id[], unsigned int power_source_type) : m_endpoint_id(++m_ep_id_counter),
                                                                                       m_period(UINT32_MAX),
...
bool ZigbeeEndpoint::setDescriptor(zb_af_endpoint_desc_t *ep_desc_p)
{
    m_endpoint_descriptor = ep_desc_p;
    return 1;
}
...
zb_af_endpoint_desc_t *ZigbeeEndpoint::endpointDescriptor() const
{
...
zb_uint8_t ZigbeeEndpoint::processCommandEP(zb_bufid_t bufid, zb_zcl_parsed_hdr_t *cmd_params)
{
    return ZB_FALSE;
}

zb_ret_t ZigbeeEndpoint::processCommandDV(zb_zcl_device_callback_param_t *cmd_params)
{
    return RET_ERROR;
}

void ZigbeeEndpoint::feedbackEffect(zb_zcl_identify_effect_value_param_t *idt_params)
{
    ...
    {
    case ZB_ZCL_IDENTIFY_EFFECT_ID_BLINK: /* Blink one time */
...
void ZigbeeEndpoint::setModelID(zb_char_t model_id[])
{
    /* Basic cluster attributes data
     * Note: the string is copied in a ZB_STRING that is a char array where the first byte is used to store the length and the "\0"
     * string terminator is not copied. */
