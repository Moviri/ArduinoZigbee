...
ZigbeeTemperatureSensor::ZigbeeTemperatureSensor(ReadTemperatureCallback callback, zb_char_t model_id[], unsigned int power_source_type) : ZigbeeEndpoint(model_id, power_source_type),
...
zb_uint8_t ZigbeeTemperatureSensor::processCommandEP(zb_bufid_t bufid, zb_zcl_parsed_hdr_t *cmd_params)
{
        ...
        cmd_params->cmd_id == ZB_ZCL_CMD_CONFIG_REPORT)
    {
        ...
        m_period = config_rep_req->u.clnt.min_interval * 1000;
    }
...
{
    int16_t curr_temp_cast = (int16_t)(m_read_temperature() * 100.0);

    zb_zcl_status_t zcl_status = zb_zcl_set_attr_val(endpointId(),
